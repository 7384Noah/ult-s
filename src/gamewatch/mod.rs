...
#[acmd_script(
    ...
    category = ACMD_GAME,
	low_priority)]
unsafe fn gnw_pivotgrab(fighter: &mut L2CAgentBase) {
    let lua_state = fighter.lua_state_agent;
	frame(fighter.lua_state_agent, /*Frames*/ 9.0);
     ...
     if macros::is_excute(fighter) {
          macros::CATCH(fighter, 0, Hash40::new("top"), 10.0, 0.0, 5.0,-4.0, Some(0.0), Some(5.0) , Some(-15.6) , *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_G);
		macros::CATCH(fighter, 1,Hash40::new("top"), 10.0, 0.0, 5.0, -2.4, Some(0.0), Some(5.0), Some(-17.2), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_A);
}
macros::game_CaptureCutCommon(fighter);
wait(fighter.lua_state_agent, /*Frames*/ 2.0);
if macros::is_excute(fighter) {
